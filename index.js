  ...
  try {
    ...
    if (err.code === 403) {
      console.warn(`⚠️ User ${chatId} memblokir bot.`);
    } else {
...
async function showMainMenu(ctx) {
  try {
    ...
    if (err.code === 403) {
      console.warn(`⚠️ User ${ctx.chat.id} memblokir bot saat showMainMenu.`);
    } else {
      console.error(`❌ Error showMainMenu:`, err);
    }
  }
}
  ...
  `Klik tombol di bawah untuk masuk ke channel:`,
  {
    ...
    bot.action(/reject_(\d+)/, (ctx) => {
      const userId = Number(ctx.match[1]);
      db.run(`DELETE FROM users WHERE id = ?`, [userId], () => {
        safeSendMessage(userId, '❌ Maaf, bukti pembayaran tidak valid. Klik /batal untuk mulai ulang.');
    ...
    bot.command('broadcast', async (ctx) => {
      ...
      db.all(`SELECT DISTINCT user_id FROM orders`, async (err, users) => {
        ...
        for (const { user_id } of users) {
          try {
            await safeSendMessage(user_id, `📢 *Broadcast dari Admin:*\n\n${message}`, {
  parse_mode: 'Markdown'
});
    ...
    bot.action(/^tendang_manual_(\d+)$/, async (ctx) => {
      ...
      db.get(`SELECT expired_at FROM orders WHERE user_id = ? ORDER BY id DESC LIMIT 1`, [userId], async (err, row) => {
        ...
        try {
         ...
         await safeSendMessage(userId, '⛔️ Akses kamu ke channel sudah dicabut oleh admin. Silahkan lakukan perpanjangan/berhenti berlangganan');
          db.run(`DELETE FROM users WHERE id = ?`, [userId]);
